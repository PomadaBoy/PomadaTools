# -*- coding: utf-8 -*-
# pyRevit - Override Graphic Settings - Solid Red Fill (Cut Patterns)

__title__   = "Solid\nRed"
__author__  = "Slobodan Vesovic"
__doc__     = "Applies solid red cut pattern override to selected elements."
...
# - HELPER: find FillPattern ElementId by name -
...
def find_fill_pattern_id(name, drafting=True):
    ...
    # first pass - match name AND pattern type
    for p in patterns:
        fp = p.GetFillPattern()
...
cut_pattern_id = find_fill_pattern_id(TARGET_PATTERN_NAME, drafting=True)

if cut_pattern_id is None:
    forms.alert(
...
# Cut Background Pattern - Solid fill + Red
ogs.SetCutBackgroundPatternId(cut_pattern_id)
ogs.SetCutBackgroundPatternColor(OVERRIDE_COLOR)

# Cut Foreground Pattern - Solid fill + Red
ogs.SetCutForegroundPatternId(cut_pattern_id)
...
with Transaction(doc, "pyRevit - Override Cut Graphic Settings") as t:
    t.Start()
    for el in elements:
        try:
